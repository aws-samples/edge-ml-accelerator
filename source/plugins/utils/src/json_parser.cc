/**
 * @json_parser.cc
 * @brief Class to parse json files
 *
 * This contains the function definitions for parsing a json file into different sections.
 *
...
using namespace std;
using namespace edgeml::utils::jsonParser;
...
namespace edgeml
{
  namespace utils
  {
    ...
    string jValue::to_string_key(int k) {
      return properties[k].first;
    }
    ...
    void jValue::replace_string(string key, string val) {
      auto it = find_if(properties.begin(), properties.end(), [&](const pair<string, jValue>& element){return element.first == key;});
      jValue v = parser::parse("\"" + val + "\"");
      if (it!=properties.end())
      {
        auto index = std::distance(properties.begin(), it);
        properties.at(index) = make_pair(key,v);
      }
      else
      {
        mpindex[key] = properties.size();
    ...
    void jValue::replace_jvalue(string key, jValue val) {
      auto it = find_if(properties.begin(), properties.end(), [&](const pair<string, jValue>& element){return element.first == key;});
      if (it!=properties.end())
      {
        auto index = std::distance(properties.begin(), it);
        properties.at(index) = make_pair(key,val);
      }
      else
      {
        mpindex[key] = properties.size();
        properties.push_back(make_pair(key, val));
      }
    }
